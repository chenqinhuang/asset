...
$(document).ready(function () {
	...
	var datatables = $("#purchaseOrderTable").DataTable({
		"oLanguage": { // 汉化
			"sProcessing": "正在加载数据...",
...
//				data.condInitiateTimeTo = $("#condInitiateTimeTo").val();
		...
		//"aaSorting": [[ 1, "asc" ]],
		"aoColumns": [
		              {"mData": "purchaseOrderId", 'sClass': '', "bSortable": false},
		              {"mData": "user.userId", 'sClass': '', "bSortable": false},
		              {"mData": "dept.deptId", 'sClass': '', "bSortable": false},
		              {
		            	  "mData": function (row, type, val) {
		            		  return row.purchaseDate.substr(0,10);
		            	  },
		            	  "bSortable": false
		              },
		              {"mData": "purchaseUse", 'sClass': '', "bSortable": false},
		              {"mData": "status", 'sClass': '', "bSortable": false},
		              {
		            	  "mData": function (row, type, val) {

		            		  var btn = "";
		            		  btn += '<a href="" class="btn btn-primary btn-sm addPurchaseOrderDetail" title="添加明细" data-id="' + row.purchaseOrderId + '" data-toggle="modal">添加明细</a>';
		            		  btn += "&nbsp;&nbsp";
		            		  btn += '<a href="" class="btn btn-primary btn-sm purchaseOrderDetail" title="查看明细" data-id="' + row.purchaseOrderId + '" data-toggle="modal">查看明细</a>';
		            		  btn += "&nbsp;&nbsp";
